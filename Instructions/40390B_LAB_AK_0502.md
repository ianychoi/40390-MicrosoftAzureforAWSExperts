...
4. Click **Compute** in the blade that comes up.
...
8. On the **Create virtual network** blade, specify the following settings and click **OK** (Note: To connect two virtual networks, they must have non-overlapping IP address space. The Azure portal provides visual clues that indicate whether the virtual network you are creating has an overlapping IP address space with any of the existing virtual networks in the same subscription):
...
- Create the first VNet Peering link
- Create the second VNet Peering link
- Verify connectivity between the two virtual networks
...
#### Task 3: Verify cnnectivity between the two virtual networks
...
16. When prompted for credentials to be used to connect to 10.0.0.4, type in **demouser** as the user name and **DemoPa$$w0rd** as the password.
...
Note: If you want to keep your lab environment in place, then you might want to consider shutting down both virtual machines. This will eliminate compute charges associated with keeping these virtual machines online. To ensure that these charges do not accrue, shut down the virtual machines from the Azure portal and ensure that they are listed on the Virtual machines blade with the Stopped (deallocated) state (rather than Stopped).
