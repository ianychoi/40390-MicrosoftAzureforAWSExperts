# Lab Answer Key:  Module 4: Virtual Networking
...
- Leveraging Resource Group Templates
- Review the Template Deployment
- Configure Routing and Remote Access for the Appliance VM

#### Task 1: Leveraging Resource Group Templates

1. Login to your Azure account at https://portal.azure.com

2. We will do a template based deployment. To save time, you will use a pre-defined template out of the Azure template quick-starts repository. Launch a browser and navigate to https://github.com/Azure/azure-quickstart-templates/tree/master/201-userdefined-routes-appliance. 

3. Click the **Deploy to Azure** button on the bottom of the page
...
- I agree to the terms and conditions stated above: enable the check box
...
Once you click  **BasicNVA**, you will be able to see the route and the subnet **FrontendSubnet** with which the route is associated. The route forces the network traffic originating from **FrontendSubnet** to flow via 10.1.1.4, which is the IP address of **vm1**, which will function in this case as a router (once you configure it later in this exercise).
If you look at the resource template file that we used avalable at https://github.com/Azure/azure-quickstart-templates/blob/master/201-userdefined-routes-appliance/azuredeploy.json, IP forwarding is enabled on the vm1.

#### Task 3: Configure Routing and Remote Access for the Appliance VM

1. Login to the **vm0 [FrontEndSubnet]** VM

2. Once in, open the command prompt by right clicking on the Windows logo in the bottom left corner and clicking **Command Prompt (Admin)**.

3. From the command prompt on **vm0 [FrontendSubnet]**, run `tracert vm2`. Based on the routing table you would expect to reach to vm2 [BackendSubnet] via vm1 [NVASubnet]. However, at this point, tracert returns `Request timed out`. There are two reasons for it:
...
4. Minimize vm0 RDP session window and connect via RDP to the **vm1** VM.
...
8. In the **Windows Firewall with Advanced Security** console, locate the **File and Printer Sharing (Echo Request – ICMPv4-In)** rule, right-click on it and click **Enable Rule**.
...
11. In the **Windows Firewall with Advanced Security** console, locate the **File and Printer Sharing (Echo Request – ICMPv4-In)** rule, right-click on it and click **Enable Rule**.
